...
class TestIGADDetector(unittest.TestCase):
    ...
    def test_score_is_zero_when_batch_matches_reference(self):
        # Large batch from the same distribution converges to near-zero curvature gap
        rng = np.random.default_rng(42)
        X_same = rng.gamma(shape=8.0, scale=0.5, size=2000)
        detector = IGADDetector(family=GammaFamily)
        detector.fit(X_same)
        score = detector.score_batch(X_same)
        # Same data -> MLE is identical -> |R_ref - R_batch| == 0 exactly
        self.assertAlmostEqual(score, 0.0, places=10)

    def test_score_is_positive_when_distributions_differ(self):
        rng = np.random.default_rng(5)
        # LogNormal with matched mean=4.0, var=2.0 — geometrically different from Gamma(8,2)
