...
def extract_rr(record_name):
    ...
    # סף אדפטיבי — אחוזון 90 של האות, לא max
    height   = np.percentile(ecg, 90)
    min_dist = int(0.25 * fs)  # 250ms — מינימום פיזיולוגי
...
if len(nsr_all) < 100 or len(afib_all) < 100:
    print("❌ עדיין לא מספיק — בדוק peak detection")
else:
    a, _, scale = stats.gamma.fit(nsr_all, floc=0)
    ks = stats.kstest(nsr_all, lambda x: stats.gamma.cdf(x, a, scale=scale))
